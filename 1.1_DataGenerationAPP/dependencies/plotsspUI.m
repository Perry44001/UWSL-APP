function plotsspUI(UIAxes, SSP, SSPType)
...
for medium = 1 : SSP.NMedia
   ...
   plot(UIAxes,  real( SSP.raw( medium ).alphaR ), SSP.raw( medium ).z, 'ko' );
   hh = plot(UIAxes,  c_eval, z_eval, 'b-' );
   hold(UIAxes, 'on')
   set( hh, 'LineWidth', 2 );
   ...
   if ( any( SSP.raw( medium ).betaR ) )
      ...
      plot(UIAxes,  real( SSP.raw( medium ).betaR ), SSP.raw( medium ).z, 'ko' );
      hh = plot(UIAxes,  c_eval, z_eval, 'r-' );
      set( hh, 'LineWidth', 2 );
   end
   
end

set( UIAxes, 'YDir', 'Reverse' )   % because view messes up the zoom feature
%axis IJ

xlabel(UIAxes, 'Sound Speed (m/s)' )
ylabel(UIAxes, 'Depth (m)' )
axis(UIAxes,'tight')
title(UIAxes, 'Sound Speed Profile')
